...
import { AcademicService, Institution, Department, Program, ClassModel, Section } from '../../services/academic.service';
...
export class AcademicStructureComponent implements OnInit {
  ...
  // forms
  instForm = this.fb.group({ id: [null], name: ['', Validators.required], type: ['school', Validators.required] });
  deptForm = this.fb.group({ id: [null], institution_id: [null, Validators.required], name: ['', Validators.required] });
  progForm = this.fb.group({ id: [null], department_id: [null, Validators.required], name: ['', Validators.required] });
  classForm = this.fb.group({ id: [null], institution_id: [null, Validators.required], program_id: [null], name: ['', Validators.required] });
  sectionForm = this.fb.group({ id: [null], class_id: [null, Validators.required], name: ['', Validators.required] });

  loading = false;
  message = '';

  constructor(private svc: AcademicService, private fb: FormBuilder) {}
  ...
  selectTab(t:any){ this.tab = t; this.message=''; if (t==='department') this.loadDepartments(); if (t==='program') this.loadPrograms(); if (t==='class') this.loadClasses(); if (t==='section') this.loadSections(); }
  ...
  // department
  loadDepartments(){ this.svc.listDepartments().subscribe(d=>this.departments=d); }
  saveDepartment(){ if(this.deptForm.invalid) return; const v=this.deptForm.value; if(v.id) this.svc.updateDepartment(v.id,v).subscribe(()=>this.afterSave('department')); else this.svc.createDepartment(v).subscribe(()=>this.afterSave('department')); }
  editDepartment(d:Department){ this.deptForm.patchValue(d); window.scrollTo({top:0,behavior:'smooth'}); }
  deleteDepartment(id:number){ if(!confirm('Delete?')) return; this.svc.deleteDepartment(id).subscribe(()=>this.loadDepartments()); }
  ...
  afterSave(kind:string){
    ...
    // reload lists
    this.loadAll(); this.loadDepartments(); this.loadPrograms(); this.loadClasses(); this.loadSections();
  }
}
