...
@Component({
  ...
  styleUrl: './assign-subject-to-teacher.component.css'
})
export class AssignSubjectToTeacherComponent {
  form!: FormGroup;
  subjects: Subject[] = [];
  ...
  public _selClassAllSubjects: any[] = [];
  ...
  // getAllSubjectData() {
  //   this.subjectService.list().subscribe(subjectData => {
  //     console.log("subjectData", subjectData);
  //   })
  // }

  getSelectedClassSubjects(_classId: number) {
    this.subjectService.getParticularClassAllSubjects(_classId).subscribe(subjects => {
      console.log("subjects", subjects);
      this._selClassAllSubjects = subjects
    })
  }
  ...
  onClassChange(event: Event) {
    ...
    this._selClassAllSubjects = [];
    this.form.patchValue({ subjects: null });

    if (classId) {
      // this({ program_id: programId });
      this.getSelectedClassSubjects(classId);
    }
  }
