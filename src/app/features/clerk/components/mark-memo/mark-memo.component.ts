...
@Component({
  selector: 'app-mark-memo',
  imports: [CommonModule, FormsModule, ReactiveFormsModule],
  templateUrl: './mark-memo.component.html',
  styleUrl: './mark-memo.component.css'
})

export class MarkMemoComponent {
  public _classes: any[] = [];
  ...
  getteacherConnectedClasses(_recUserId: number) {
    ...
    this._academicsService.listClasses().subscribe(data => {
      if (data.length > 0) {
        data.map(dt => {
          if (dt.id !== null && dt.id !== undefined) {
            this.selTeacherClassId.push(dt.id)
          }
        })
      }
    })
  ...
  navigateTofillMark(classId: any) {
    this._router.navigate(['clerk/show-mark', classId]);
  }
}
